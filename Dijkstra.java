...
public class Dijkstra {
		...
		while (true) {
			...
			for (Edge e : min.adjacentEdges) {
				...
				if (!target.known) {
					...
					if (min.distance + e.distance < target.distance) {
	...
	private Vertex findMinUnknown() {
		Vertex minVertex = null;
		for (Vertex vertex : vertexNames.values()) {
			if (!vertex.known && (minVertex == null || vertex.distance < minVertex.distance)) {
				minVertex = vertex;
			}
		}
	...
	public List<Edge> getDijkstraPath(String s, String t) {
		...
		for (Vertex currVertex = vertexNames.get(t); currVertex != firstVertex; currVertex = currVertex.prev) {
			if (currVertex.prev == null) {
				break;
			}
			for (Edge e : currVertex.prev.adjacentEdges) {
				if (e.target == currVertex) {
					path.add(0, e);
					break;
				}
			}
